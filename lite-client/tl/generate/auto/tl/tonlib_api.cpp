...
raw_accountState::raw_accountState(std::int64_t balance_, std::string const &code_, std::string const &data_, object_ptr<internal_transactionId> &&last_transaction_)
  : balance_(balance_)
  , code_(std::move(code_))
  , data_(std::move(data_))
  , last_transaction_(std::move(last_transaction_))
{}
...
void raw_accountState::store(td::TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_bytes_field("data", data_);
    if (last_transaction_ == nullptr) { s.store_field("last_transaction", "null"); } else { last_transaction_->store(s, "last_transaction"); }
    s.store_class_end();
  }
}
...
createNewKey::createNewKey(td::SecureString &&local_password_, td::SecureString &&mnemonic_password_)
  : local_password_(std::move(local_password_))
